...
if [[ -z "$settings_file" ]]; then
  settings_file="$(mktemp /tmp/chengdd-marketing-mvn-settings.XXXXXX)"
...
export CDD_MARKETING_DB_URL="${CDD_MARKETING_DB_URL:-jdbc:mysql://127.0.0.1:${mysql_port}/${mysql_database}?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai&useSSL=false}"
export CDD_MARKETING_DB_USERNAME="${CDD_MARKETING_DB_USERNAME:-$mysql_username}"
...
mvn -q -s "$settings_file" "-Dmaven.repo.local=$work_repo" -f "${parent_root}/pom.xml" \
  -pl cdd-marketing-service -am \
  -DskipTests install

mvn -q -s "$settings_file" "-Dmaven.repo.local=$work_repo" -f "${parent_root}/cdd-marketing-service/pom.xml" \
  spring-boot:run \
  -Dspring-boot.run.arguments="--server.port=${CDD_MARKETING_SERVER_PORT:-8086}"
