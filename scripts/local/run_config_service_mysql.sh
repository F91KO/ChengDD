...
if [[ -z "$settings_file" ]]; then
  settings_file="$(mktemp /tmp/chengdd-config-mvn-settings.XXXXXX)"
...
export CDD_CONFIG_DB_URL="${CDD_CONFIG_DB_URL:-jdbc:mysql://127.0.0.1:${mysql_port}/${mysql_database}?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai&useSSL=false}"
export CDD_CONFIG_DB_USERNAME="${CDD_CONFIG_DB_USERNAME:-$mysql_username}"
...
mvn -q -s "$settings_file" "-Dmaven.repo.local=$work_repo" -f "${parent_root}/cdd-config-service/pom.xml" \
  spring-boot:run \
  -Dspring-boot.run.arguments="--server.port=${CDD_CONFIG_SERVER_PORT:-8089}"
