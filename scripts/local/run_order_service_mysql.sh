...
if [[ -z "$settings_file" ]]; then
  settings_file="$(mktemp /tmp/chengdd-order-mvn-settings.XXXXXX)"
...
export CDD_ORDER_DB_URL="${CDD_ORDER_DB_URL:-jdbc:mysql://127.0.0.1:${mysql_port}/${mysql_database}?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai&useSSL=false}"
export CDD_ORDER_DB_USERNAME="${CDD_ORDER_DB_USERNAME:-$mysql_username}"
...
record_backend_runtime_state "$repo_root" "order-service" "cdd-order-service" "$order_port"

mvn -q -s "$settings_file" "-Dmaven.repo.local=$work_repo" -f "${parent_root}/pom.xml" \
  -pl cdd-order-service -am \
  spring-boot:run \
  -Dspring-boot.run.arguments="--server.port=${order_port}"
