...
mvn -q -s "$settings_file" "-Dmaven.repo.local=$work_repo" -f "${parent_root}/pom.xml" \
  -pl cdd-report-service -am \
  -DskipTests install

mvn -q -s "$settings_file" "-Dmaven.repo.local=$work_repo" -f "${parent_root}/cdd-report-service/pom.xml" \
  spring-boot:run \
  -Dspring-boot.run.arguments="--server.port=${CDD_REPORT_SERVER_PORT:-8088}"
