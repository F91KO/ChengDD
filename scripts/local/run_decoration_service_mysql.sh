...
if [[ -z "$settings_file" ]]; then
  settings_file="$(mktemp /tmp/chengdd-decoration-mvn-settings.XXXXXX)"
...
export CDD_DECORATION_DB_URL="${CDD_DECORATION_DB_URL:-jdbc:mysql://127.0.0.1:${mysql_port}/${mysql_database}?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai&useSSL=false}"
export CDD_DECORATION_DB_USERNAME="${CDD_DECORATION_DB_USERNAME:-$mysql_username}"
export CDD_DECORATION_DB_PASSWORD="${CDD_DECORATION_DB_PASSWORD:-$mysql_password}"
...
mvn -q -s "$settings_file" "-Dmaven.repo.local=$work_repo" -f "${parent_root}/cdd-decoration-service/pom.xml" \
  spring-boot:run \
  -Dspring-boot.run.arguments="--server.port=${CDD_DECORATION_SERVER_PORT:-8083}"
