...
while IFS='|' read -r service_name _service_module _service_port launcher; do
  ...
  rm -f "$(backend_runtime_state_file "$repo_root" "$service_name")"
  ...
  if ! wait_for_runtime_service_health "$repo_root" "$service_name" "$_service_module" "$_service_port" "$launcher_pid"; then
